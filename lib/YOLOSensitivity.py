...
if __name__ == '__main__':
    ...
    data_path1 = "motchallenge/mot17/train/*"  # "/local/a/cam2/data/motchallenge"
    data_path2 = "motchallenge/mot17/test/*"  # "/local/a/cam2/data/motchallenge"
    ...
    # norm_layer1 = [gradient.norm().detach().cpu().numpy() for gradient in gradients1 if gradient is not None]
    # norm_layer2 = [gradient.norm().detach().cpu().numpy() for gradient in gradients2 if gradient is not None]

    # np.savetxt("pair_compare/incr_1_layer.csv", np.array(norm_layer1), delimiter=",")
    # np.savetxt("pair_compare/incr_2_layer.csv", np.array(norm_layer2), delimiter=",")
